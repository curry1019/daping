...
import qs from 'qs';
import {jsonp,startLoading,endLoading} from '@/axios/base/utils';
...
const baseUrl = (function () {
	switch (process.env.NODE_ENV != 'production' ? 'development' : 'production') {// 代理线上数据or生产
		case 'development':
			return 'http://192.168.199.209:9999/';
			//return 'https://xhc.test.51yuqian.net:1443/qdj/api/';
		case 'production':
			return 'api/';
		default:
...
$http.interceptors.request.use((config) => {
    ...
    if(config.url.indexOf('auth/login') == -1){
      let token=localStorage.getItem("access-user")!=null?JSON.parse(localStorage.getItem("access-user")).data.token:"";
      if(token!==""){
